...
for(var i in selectOption)
{
	if(i!='请选择省份')
	{
		$('.sheng dl').append('<dd><a href="javascript:void(0)">'+i+'</a></dd>');
	}
}

$('.agent_select').mouseenter(function(){
	$(this).find('dl').show();
});
$('.agent_select').mouseleave(function(){
	$(this).find('dl').hide();
});
$('.agent_select').on('click','dd',function(){
	$(this).parents('.agent_select').find('input').val($(this).text());
	$(this).parents('dl').hide();
	if($(this).parents('.agent_select').hasClass('sheng'))
	{
		$('.sheng input').change();
	}
});
$('.sheng input').change(function(){
	var jsonCity=selectOption[$(this).val()]['values'];
	var cityHtml='';
	for(var j in jsonCity)
	{
		cityHtml+='<dd><a href="javascript:void(0)">'+j+'</a></dd>';
	}
	$('.shi dl').html(cityHtml);
	$('.shi input').val($('.shi dl dd:first').text());
});